from kamene.all import Ether, ARP, IP, ICMP, conf, sr, srp, sr1, TCP, UDP
...
class PortScanner():
    ...
    def syn_scan_port(self, destination_ip:str, destination_port:int, timeout_length:int):
        """
            \'syn_scan_port\' will do a scan on target device port.
        ...
        """
        response, noresponse = sr(IP(dst=destination_ip)/TCP(dport=destination_port, flags='S', options=[('Timestamp',(random.randint(0, 10),random.randint(10, 20)))]), timeout=10, verbose=0)
        if noresponse:
            return (False, "Port is closed", destination_port)
        match response[0][1].flags:
            case 2:
                return (True, "Port is open", destination_port)
            case _:
                return (True, "Port state is unknown", destination_port)
        return PortScannerError(f"IP address {destination_ip} did not respond on port {destination_port}")
    def ack_scan_port(self, destination_ip:str, destination_port:int, timeout_length:int):
        """
            ...
            Below are the arguments explained:
                ...
                \'timeout_length\':int is an argument that will set the timeout_length for the TCP connection
        """
        response, noresponse = sr(IP(dst=destination_ip)/TCP(dport=destination_port, flags='A', options=[('Timestamp',(random.randint(0, 10),random.randint(10, 20)))]), timeout=10, verbose=0)
        if noresponse:
    ...
    def list_port_lists(self):
        ...
        return return_dict
    def managed_scan_port(self, destination_ip:str, connection_type:str, ports_to_scan:tuple, return_dictionary:dict, process_id:int):
        """
        ...
        for port in ports_to_scan:
            response, noresponse = sr(IP(dst=destination_ip)/TCP(dport=port, flags=connection_type, options=[('Timestamp',(random.randint(0, 10),random.randint(10, 20)))]), timeout=10, verbose=0)
            if response:
                match response[0][1].flags:
                    case 2:
                        responded_ports.append((True, "Port is open", port))
                    case _:
                        responded_ports.append((True, "Port is open", port))
        ...
        if use_custom_port is True:
            ...
            if syn_scan is True:
                for process_id in range(1, self.MAX_PROCESSES):
                    process = Process(target=PortScanner().managed_scan_port, args=(target_ip, 'S', custom_list[last_ports_count:(last_ports_count + ports_per_process)], return_dictionary, process_id))
                    process.start()
                    scan_processes.append(process)
                    last_ports_count += ports_per_process
            else:
                for process_id in range(1, self.MAX_PROCESSES):
                    process = Process(target=PortScanner().managed_scan_port, args=(target_ip, 'A', custom_list[last_ports_count:(last_ports_count + ports_per_process)], return_dictionary, process_id))
                    process.start()
                    scan_processes.append(process)
                    last_ports_count += ports_per_process
        else:
            if syn_scan is True:
                match ports_to_scan:
                    case 0:
                        ...
                        for process_id in range(1, self.MAX_PROCESSES):
                            process = Process(target=PortScanner().managed_scan_port, args=(target_ip, 'S', PortScanner().SML_LIST[last_ports_count:(last_ports_count + ports_per_process)], return_dictionary, process_id))
                            process.start()
                            scan_processes.append(process)
                            last_ports_count += ports_per_process
                    case 1:
                        ...
                        for process_id in range(1, self.MAX_PROCESSES):
                            process = Process(target=PortScanner().managed_scan_port, args=(target_ip, 'S', PortScanner().MED_LIST[last_ports_count:(last_ports_count + ports_per_process)], return_dictionary, process_id))
                            process.start()
                            scan_processes.append(process)
                            last_ports_count += ports_per_process
                    case _:
                        raise PortScannerError(f"Incompatable input scan type \'{ports_to_scan}\'!")
            elif syn_scan is False:
                match ports_to_scan:
                    case 0:
                        ports_per_process = int(len(self.SML_LIST)/self.MAX_PROCESSES)
                        for process_id in range(1, self.MAX_PROCESSES):
                            process = Process(target=PortScanner().managed_scan_port, args=(target_ip, 'A', PortScanner().SML_LIST[last_ports_count:(last_ports_count + ports_per_process)], return_dictionary, process_id))
                            process.start()
                            scan_processes.append(process)
                            last_ports_count += ports_per_process
                    case 1:
                        ports_per_process = int(len(self.MED_LIST)/self.MAX_PROCESSES)
                        for process_id in range(1, self.MAX_PROCESSES):
                            process = Process(target=PortScanner().managed_scan_port, args=(target_ip, 'A', PortScanner().MED_LIST[last_ports_count:(last_ports_count + ports_per_process)], return_dictionary, process_id))
                            process.start()
                            scan_processes.append(process)
                            last_ports_count += ports_per_process
                    case _:
                ...
                raise PortScannerError("Incompatable syn_scan type!")
        for process in scan_processes:
            process.join()
        for key in return_dictionary.keys():
            for port in return_dictionary[key]:
